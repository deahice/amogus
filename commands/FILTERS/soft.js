...
if (!args[0]) {
			...
			const msg = await message.channel.send(`<a:Loading:909335917779648512> Turning on **soft**.`);
			const embed = new MessageEmbed()
				.setDescription('<a:yes:909104496318222387> Turned on **soft**')
				.setColor("BLACK");
			await delay(2000);
	...
	player.setEQ(Array(13).fill(0).map((n, i) => ({
        ...
        const msg = await message.channel.send(`<a:Loading:909335917779648512> Turning off **soft mode**.`);
			const embed = new MessageEmbed()
				.setDescription('<a:yes:909104496318222387> Turned off **soft mode**')
				.setColor("BLACK");
			await delay(2000);
			return msg.edit('', embed);
		}
