   ...
   if (channel.id !== player.voiceChannel) {
    ...
    return message.channel.send(embed)
		...
		const embed = new MessageEmbed()
			.setDescription('<a:yes:909104496318222387> Turned on **earrape**. ')
			.setColor("BLACK");
		return message.channel.send(embed);
		...
		player.clearEffects()
		const msg = await message.channel.send(`<a:Loading:909335917779648512> Turning off **earrape**`);
			const embed = new MessageEmbed()
				.setDescription('<a:yes:909104496318222387> Turned off **earrape**')
				.setColor("BLACK");
			await delay(2000);
