 ...
 let play = message.client.manager.players.get(message.guild.id)
  ...
  if (!args.length) {
    let embed = new MessageEmbed()
      .setColor("000000")
      .setDescription(`<:error:888328080110800926> To play a song you need to specify the song name or url`)
  ...
  if (!play) {
    ...
    if (!channel.joinable) {
      let embed = new MessageEmbed()
        ...
        .setDescription("<:error:888328080110800926> Iam umable to join your voice channel")
      return message.channel.send(embed)
    }
...
let msg = message.channel.send(`searching  for \`${search}\``).then(m => m.delete({ timeout: 5000 }));
  ...
  try {
    ...
    let embed = new MessageEmbed()
      ...
      .setDescription(`<:error:888328080110800926> I can't find result about \`${search}\``
       )
    return message.channel.send(embed)
  }
  ...
  switch (res.loadType) {
    case 'NO_MATCHES':
      ...
      let embed = new MessageEmbed()
        ...
        .setDescription(`<:error:888328080110800926> I can't find results about \`${search}\``)
      return message.channel.send(embed)
