...
const { getStations } = require('radio-browser')
  ...
  run: async(client,message,args) => {
    ...
    getStations({
			...
			.then(async data => {
				if (!data[0]) return message.channel.send('<:error:888328080110800926> No radio found with that name');
				...
				try {
					...
					return message.reply('<:error:888328080110800926> You didn\'t choose a song in time.');
				}
				...
				if (first.toLowerCase() === 'cancel') {
					...
					return message.channel.send('<a:yes:909104496318222387> Cancelled selection.');
				}

				const index = Number(first) - 1;
				if (index < 0 || index > max - 1) return message.reply(`<:error:888328080110800926> The number you provided was too small or too big (1-${max}).`);
